...
![Dashboard of cell towers by radio type in mcc 204](./superset-cell-tower-dashboard.png)
...
![Load cell towers dataset](./cloud-load-data-sample.png)
...
<ConnectionDetails />
  ...
  ![Add a database](./superset-add.png)

  Choose **ClickHouse Connect** from the list:

  ![Choose clickhouse connect as database type](./superset-choose-a-database.png)
...
:::
  ...
  ![Add ClickHouse as a Superset data source](./superset-connect-a-database.png)
...
![Add cell_towers table as a dataset](./superset-add-dataset.png)
...
![Create a map in Superset](./superset-create-map.png)
...
![Specify longitude and latitude fields](./superset-lon-lat.png)

Add a filter with `mcc` = `204` (or substitute any other `mcc` value):

![Filter on MCC 204](./superset-mcc-204.png)
...
![Filter on radio = UMTS](./superset-radio-umts.png)

This is the full configuration for the chart that filters on `radio = 'UMTS'` and `mcc = 204`:

![Chart for UMTS radios in MCC 204](./superset-umts-netherlands.png)
